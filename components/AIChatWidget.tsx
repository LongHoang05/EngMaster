...
export default function AIChatWidget({
  ...
  const vocabContext = vocabularies
    .map(
      (v) =>
        `- ${v.word}: ${Array.isArray(v.meanings) ? v.meanings[0] : v.meanings}`,
    )
    .join("\n");
  ...
  const handleTranslate = async (msgId: string, text: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Lỗi API Dịch:", errData);
        toast.error("Không thể dịch nội dung này. Vui lòng thử lại!");
      }
    } catch (err) {
      console.error("Lỗi mạng khi gọi API Dịch:", err);
      toast.error("Lỗi mạng khi gọi API Dịch.");
    } finally {
      ...
      setMessages((prev) => {
        ...
        if (last.id === aiMsgId && !last.content.trim()) {
          updated[updated.length - 1] = {
            ...last,
            content:
              "⚠️ (Hệ thống) AI Provider hiện đang quá tải hoặc từ chối kết nối. Hệ thống xoay tua sẽ tự động chọn Model khác ở lượt tiếp theo. Vui lòng gửi lại tin nhắn!",
          };
        }
        return updated;
      });
  ...
  return (
    <div className="fixed bottom-[5.5rem] left-4 right-4 sm:bottom-[6.5rem] sm:left-auto sm:right-6 z-[100] flex flex-col items-end pointer-events-none">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="bg-white pointer-events-auto border-2 border-indigo-100 shadow-2xl rounded-2xl sm:rounded-3xl w-full sm:w-[400px] h-[65vh] sm:h-[500px] max-h-[600px] flex flex-col overflow-hidden mb-4"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
              {messages.map((m) => (
                ...
                >
                  ...
                  <div
                    className={`px-4 py-3 rounded-2xl max-w-[80%] text-sm shadow-sm ${m.role === "user" ? "bg-indigo-500 text-white rounded-tr-sm" : "bg-white text-slate-700 border border-slate-100 rounded-tl-sm"}`}
                  >
      ...
      <button
        ...
        className="w-14 h-14 pointer-events-auto bg-indigo-600 hover:bg-indigo-700 text-white rounded-full flex items-center justify-center transition-all hover:scale-110 active:scale-95 group relative shrink-0"
      >
        {isOpen ? (
          <X size={24} />
        ) : (
