...
function checkAnswer(userAnswer, correctAnswer) {
  ...
  var parts = normCorrect.split(/[,;\/]/).map(function(s) { return s.trim().toLowerCase(); });
...
self.addEventListener("push", function(event) {
  var rawData = {};
  try { rawData = event.data ? event.data.json() : {}; } catch(e) { return; }

  // Try to find our quiz data in various OneSignal formats
  var quizData = null;
  if (rawData.custom && rawData.custom.a && rawData.custom.a.type === "quiz_hybrid") {
    quizData = rawData.custom.a;
  } else if (rawData.data && rawData.data.type === "quiz_hybrid") {
    quizData = rawData.data;
  } else if (rawData.additionalData && rawData.additionalData.type === "quiz_hybrid") {
...
self.addEventListener("notificationclick", function(event) {
  ...
  if (data.type === "quiz_hybrid" || data.type === "quiz") {
    var legacyActionMap = {
      "btn_A_v20": 0, "btn_B_v20": 1,
      "btn_A_v19": 0, "btn_B_v19": 1
    };
