...
async function run() {
  // 1. Get Existing Series
  const { data: seriesData, error: seriesError } = await supabase.from('movie_series').select('id').ilike('title', '%Peppa%').single();

  if (seriesError || !seriesData) {
    console.error("Cannot find Peppa series:", seriesError);
    return;
  }
  ...
  // 2. Get Existing Episodes
  const { data: existingEpisodes } = await supabase.from('movie_episodes').select('youtube_id').eq('series_id', seriesId);
  const existingIds = new Set(existingEpisodes?.map(e => e.youtube_id) || []);

  const playlistId = 'PLFEgnf4tmQe_YHkXH2p7X-dqnBzR0FIJn';
  console.log("Đang lấy thông tin Playlist:", playlistId);
  const list = await yts({ listId: playlistId });
  
  const newVideos = list.videos.filter(v => !existingIds.has(v.videoId));
  console.log(`Đã tìm thấy ${list.videos.length} video từ playlist. Lọc ra ${newVideos.length} video MỚI. Bắt đầu xử lý...`);
  ...
  console.log(`\n\n[XONG] Quá trình hoàn tất. Đã cào và lưu thành công ${addedCount} tập phim Peppa Pig mới.`);
}
